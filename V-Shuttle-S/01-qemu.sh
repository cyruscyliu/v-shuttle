...
# cp ./clangcovdump.h $QEMU_DIR/include
...
# compile QEMU
pushd $QEMU_DIR
CC=afl-gcc CXX=afl-g++ ./configure --disable-werror --disable-sanitizers --enable-gcov --target-list="x86_64-softmmu"
# make CFLAGS="-fprofile-instr-generate -fcoverage-mapping" -j$(nproc) x86_64-softmmu/all
make CONFIG_FUZZ=y -j$(nproc) x86_64-softmmu/all
popd
